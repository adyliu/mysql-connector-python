...
class MySQLInstallBase(object):
    ...
    def __init__(self, basedir, optionFile=None):
        ...
        if optionFile is not None and os.access(optionFile,0):
            MySQLBootstrapError("Option file not accessible: {}".format(
                optionFile))
    ...
    def _init_mysql_install(self):
        ...
        if self._bindir is None or self._sbindir is None:
            raise MySQLBootstrapError(
                "MySQL binaries not found under {}".format(self._basedir))
...
class MySQLBootstrap(MySQLInstallBase):
    ...
    def _create_directories(self):
        ...
        Raises MySQLBootstrapError when something fails.
        """
        logger.debug("Creating {0} {0}/mysql and {0}/test".format(
            self._datadir))
        try:
    ...
    def bootstrap(self):
        ...
        try:
            ...
            for f in script_files:
                logger.debug("Reading SQL from '{}'".format(f))
                fp = open(os.path.join(self._sharedir,f),'r',encoding='utf8')
                sql += fp.readlines()
                fp.close()
            sql += self.extra_sql
...
class MySQLd(MySQLInstallBase):
    ...
    def _get_cmd(self):
        cmd = [
            os.path.join(self._sbindir, EXEC_MYSQLD),
            "--defaults-file={}".format(self._optionFile)
        ]
...
class MySQLInit(object):
    ...
    def bootstrap(self):
        ...
        try:
            ...
            logger.error("Failed bootstrapping MySQL: {}".format(e))
    ...
    def start(self):
        ...
        try:
            ...
            logger.error("Failed starting MySQL server: {}".format(err))
            if self._debug is True:
                raise
            sys.exit(1)

    def stop(self):
        logger.info("Stopping MySQL server")
        try:
            if not self._server.stop():
                try:
                    pid = int(open(self._pid_file, 'r').readline().strip())
                    os.kill(pid, signal.SIGKILL)
                except IOError:
                    pass
        except MySQLdError as e:
    ...
    def remove(self):
        try:
            ...
            logger.debug("Failed removing {}: {}".format(self._topdir, e))
            if self._debug is True:
                raise
            else:
                pass
        else:
            logger.info("Removed {}".format(self._topdir))
