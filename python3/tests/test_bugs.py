...
class Bug551533and586003(tests.MySQLConnectorTests):
    ...
    def setUp(self):
        ...
        self.tbl = 'Bug551533'
        self.c.execute("DROP TABLE IF EXISTS {table}".format(table=self.tbl))
        self.c.execute((
                "CREATE TABLE {table} ("
                "id INT AUTO_INCREMENT KEY, "
    ...
    def tearDown(self):
        try:
            c = self.db.cursor()
            c.execute("DROP TABLE IF EXISTS {table}".format(table=self.tbl))
        except:
            pass
        self.db.close()
    ...
    def setUp(self):
        ...
        self.admin_cnx.cmd_query(
            "GRANT ALL ON {db}.* TO 'ssluser'@'{host}' REQUIRE SSL".format(
                db=config['database'], host=tests.MYSQL_CONFIG['host']))
...
class BugOra16316049(tests.MySQLConnectorTests):
    ...
    def test_ssl(self):
        ...
        config['ssl_ca'] = os.path.abspath(
            os.path.join(tests.SSL_DIR, 'tests_casdfasdfa_cert.pem'))
...
class BugOra16662920(tests.MySQLConnectorTests):
    ...
    def test_buffered_raw(self):
        ...
        cur.execute("SELECT * FROM t1 ORDER BY c1")
        self.assertEqual((b'1', b'a'), cur.fetchone())
        exp = [(b'2', b'c'), (b'4', b'd'), (b'3', b'e')]
        self.assertEqual(exp, cur.fetchmany(3))
        exp = [(b'6', b'f'), (b'5', b'g')]
        self.assertEqual(exp, cur.fetchall())
        cur.close()
...
class BugOra17041412(tests.MySQLConnectorTests):
    ...
    def setUp(self):
        ...
        cur.executemany(
            "INSERT INTO {table} (c1) VALUES (%s)".format(
                table=self.table_name), self.data)
        self.cnx.commit()
    ...
    def test_many(self):
        ...
        cur = self.cnx.cursor()
        cur.execute("SELECT * FROM {table} WHERE c1 > 1000".format(
            table=self.table_name))
...
class BugOra16656621(tests.MySQLConnectorTests):
    ...
    def test_rollback(self):
        self.cur.execute("INSERT INTO t1 (c1) VALUES ('a'),('b'),('c')")
        self.cnx.commit()

        self.cur.execute("SELECT * FROM t1")
        try:
            self.cnx.rollback()
...
class BugOra16933795(tests.MySQLConnectorTests):
    ...
    def test_error(self):
        ...
        error = errors.Error(msg=exp, errno=-1024)
        self.assertEqual(exp, error.msg)
        self.assertEqual("-1024: " + exp, str(error))

        exp = "Unknown MySQL error"
        error = errors.Error(errno=2000)
        self.assertEqual(exp, error.msg)
...
class BugOra17022399(tests.MySQLConnectorTests):
    ...
    """
    def setUp(self):
        self.config = self.getMySQLConfig()
    ...
    def test_execute(self):
        ...
        try:
            ...
            if os.name == 'nt':
                self.assertTrue('10038' in str(err))
            else:
...
class BugOra17422299(tests.MySQLConnectorTests):
    ...
    def tearDown(self):
        ...
        if not tests.MYSQL_SERVER.check_running():
            tests.MYSQL_SERVER.start()

            if not tests.MYSQL_SERVER.wait_up():
    ...
    def test_shutdown(self):
        ...
        if tests.MYSQL_SERVER.wait_down():
...
class BugOra17414258(tests.MySQLConnectorTests):
    ...
    def tearDown(self):
        ...
        del _CONNECTION_POOLS[self.config['pool_name']]
    ...
    def test_poolsize(self):
        ...
        self.assertRaises(errors.PoolError,
                          connect, **newconfig)
