...
class MySQLConnectionTests(tests.MySQLConnectorTests):
    ...
    def test__handle_result(self):
        ...
        self.cnx._socket.sock.reset()
        packet = (
            '\x22\x00\x00\x01\xfb\x70\x79\x74\x68\x6f\x6e\x32\x2f'
            '\x74\x65\x73\x74\x73\x2f\x64\x61\x74\x61\x2f\x6c\x6f'
            '\x63\x61\x6c\x5f\x64\x61\x74\x61\x2e\x63\x73\x76')
        self.cnx._socket.sock.add_packet(
            '\x37\x00\x00\x04\x00\x06\x00\x01\x00\x00\x00\x2f\x52'
            '\x65\x63\x6f\x72\x64\x73\x3a\x20\x36\x20\x20\x44\x65'
            '\x6c\x65\x74\x65\x64\x3a\x20\x30\x20\x20\x53\x6b\x69'
            '\x70\x70\x65\x64\x3a\x20\x30\x20\x20\x57\x61\x72\x6e'
            '\x69\x6e\x67\x73\x3a\x20\x30')
        exp = {
        ...
        exp = [
            ('\x47\x00\x00\x04\x31\x09\x63\x31\x5f\x31\x09\x63\x32'
             '\x5f\x31\x0a\x32\x09\x63\x31\x5f\x32\x09\x63\x32\x5f'
             '\x32\x0a\x33\x09\x63\x31\x5f\x33\x09\x63\x32\x5f\x33'
             '\x0a\x34\x09\x63\x31\x5f\x34\x09\x63\x32\x5f\x34\x0a'
             '\x35\x09\x63\x31\x5f\x35\x09\x63\x32\x5f\x35\x0a\x36'
             '\x09\x63\x31\x5f\x36\x09\x63\x32\x5f\x36'),
            '\x00\x00\x00\x05']
        self.assertEqual(exp, self.cnx._socket.sock._client_sends)

        # Column count is invalid (is None)
        packet = '\x01\x00\x00\x01\xfa\xa3\x00\xa3'
        self.assertRaises(errors.InterfaceError,
    ...
    def test_config(self):
        ...
        cnx._ssl = {}
        try:
            cnx.config(ssl_ca=exp['ca'])
        except AttributeError, err:
            errmsg = str(err)
            self.assertTrue('ssl_cert' in errmsg and 'ssl_key' in errmsg)
        else:
            self.fail("Testing SSL attributes failed.")

        cnx._ssl = {}
        try:
            cnx.config(ssl_cert=exp['cert'])
        except AttributeError, err:
            errmsg = str(err)
            self.assertTrue('ssl_key' in errmsg)
        else:
            self.fail("Testing SSL attributes failed.")

        cnx._ssl = {}
        try:
            cnx.config(ssl_verify_cert=True)
        except AttributeError, err:
            errmsg = str(err)
            self.assertTrue('ssl_key' in errmsg and 'ssl_cert' in errmsg
                            and 'ssl_ca' in errmsg)
        else:
            self.fail("Testing SSL attributes failed.")
    ...
    def test_connect(self):
        ...
        cnx = connection.MySQLConnection()
        config['host'] = tests.MYSQL_CONFIG['host']
        try:
            cnx.connect(**config)
            cnx.close()
        except Exception as err:
            self.fail("Failed connecting to '{0}': {1}".format(
                config['host'], str(err)))
    ...
    def test_reconnect(self):
        ...
        self.assertTrue(result > 2 and result < 12,
                        "2 <= result < 12, was %d" % result)
    ...
    def test__send_data(self):
        ...
        fp = StringIO.StringIO(csv_data)
        self.cnx._socket.sock.reset()
        self.cnx._socket.sock.add_packet(OK_PACKET)
        exp = [
            "\x20\x00\x00\x02\x31\x09\x68\x61\x6d\x09\x27\x68\x61\x6d"
            "\x20\x73\x70\x61\x6d\x27\x0a\x32\x09\x66\x6f\x6f\x09\x27"
            "\x66\x6f\x6f\x20\x62\x61\x72\x27"
            ]
    ...
    def test__handle_binary_ok(self):
        ...
        # Raise an error
        packet = (
            '\x2a\x00\x00\x01\xff\x19\x05\x23\x34\x32\x30\x30\x30\x46\x55'
            '\x4e\x43\x54\x49\x4f\x4e\x20\x74\x65\x73\x74\x2e\x53\x50\x41'
            '\x4d\x20\x64\x6f\x65\x73\x20\x6e\x6f\x74\x20\x65\x78\x69\x73\x74'
            )
        self.assertRaises(errors.ProgrammingError,
    ...
    def test_cmd_stmt_prepare(self):
        ...
        stmt = "SELECT CONCAT(?, ?) AS c1"
        self.cnx._socket.sock.add_packets([
            '\x0c\x00\x00\x01\x00\x01\x00\x00\x00\x01\x00\x02\x00\x00\x00\x00',
            '\x17\x00\x00\x02\x03\x64\x65\x66\x00\x00\x00\x01\x3f\x00\x0c'\
            '\x3f\x00\x00\x00\x00\x00\xfd\x80\x00\x00\x00\x00',
            '\x17\x00\x00\x03\x03\x64\x65\x66\x00\x00\x00\x01\x3f\x00\x0c'\
            '\x3f\x00\x00\x00\x00\x00\xfd\x80\x00\x00\x00\x00',
            EOF_PACKET,
            '\x18\x00\x00\x05\x03\x64\x65\x66\x00\x00\x00\x02\x63\x31\x00'\
            '\x0c\x3f\x00\x00\x00\x00\x00\xfd\x80\x00\x1f\x00\x00',
            EOF_PACKET
            ])
        exp = {
            'num_params': 2,
        ...
        self.cnx._socket.sock.reset()
        self.cnx._socket.sock.add_packets([
            '\x0c\x00\x00\x01\x00\x01\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00'
            ])
        exp = {
            'num_params': 0,
            'statement_id': 1,
    ...
    def test__handle_binary_result(self):
        ...
        self.assertRaises(errors.InterfaceError,
                          self.cnx._handle_binary_result, '\x00\x00\x00')
        ...
        self.cnx._socket.sock.reset()
        self.cnx._socket.sock.add_packets([
            ('\x18\x00\x00\x02\x03\x64\x65\x66\x00\x00\x00\x02\x63\x31\x00'
             '\x0c\x21\x00\x09\x00\x00\x00\xfd\x01\x00\x00\x00\x00'),
            EOF_PACKET,
            ])
        exp = (
            1, 
        ...
        self.assertEqual(
            exp, self.cnx._handle_binary_result('\x01\x00\x00\x01\x01'))
    ...
    def test_cmd_stmt_close(self):
        ...
        stmt_info = self.cnx.cmd_stmt_prepare("SELECT ? as c1")
