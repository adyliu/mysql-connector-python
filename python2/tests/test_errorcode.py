...
class ErrorCodeTests(tests.MySQLConnectorTests):
	def test__GENERATED_ON(self):
		...
		try:
			ymd = [ int(v) for v in errorcode._GENERATED_ON.split('-', 3)]
			generatedon = date(*ymd)
		except ValueError as err:
			self.fail(err)

		delta = (datetime.now().date() - generatedon).days
		self.assertTrue(delta < 120,
						"errorcode.py is more than 120 days old (%d)" % delta)
	...
	def _check_code(self, code, nr):
		try:
			self.assertEqual(getattr(errorcode, code), nr)
	...
	def test_server_error_codes(self):
		...
		for code, nr in cases.items():
			self._check_code(code, nr)
